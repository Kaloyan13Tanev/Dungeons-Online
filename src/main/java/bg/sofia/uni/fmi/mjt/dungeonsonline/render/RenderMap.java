...
import static bg.sofia.uni.fmi.mjt.dungeonsonline.temp.TempMap.GAME_MAP;
import static bg.sofia.uni.fmi.mjt.dungeonsonline.terminal.TerminalManager.terminal;

public class RenderMap {

    //private static final GameMap GAME_MAP;
    ...
    public void render() {
        ...
        for (int i = 0; i < GAME_MAP.getHeight(); i++) {
            ...
            for (int j = 0; j < GAME_MAP.getWidth(); j++) {
                ...
                if (GAME_MAP.getTile(i, j).isWalkable()) {
                    RenderTile.render((WalkableTile) GAME_MAP.getTile(i, j), row + 1, col + 1);
                } else {
